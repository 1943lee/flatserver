    ...
    public static String converterToFirstSpell(String chines) {
        ...
        for (int i = 0; i < nameChar.length; i++) {
            if (nameChar[i] > 128) {
                ...
                pinyinName.append(Character.toUpperCase(nameChar[i]));
            }
            pinyinName.append(" ");
        }
    ...
    public static String convertToSpellAll(String str,int maxLength){
        ...
        for(int i = 0;i<multiStr.length;i++) {
            ...
            if (!name.isEmpty()) {
                ...
                if(!ComConvert.toString(result).isEmpty()) {
                    result.append(";" + original);
                }
                else{
                    result.append(original);
                }
                String firstSpell = Pinyin4jUtil.converterToFirstSpell(name);
                if (!firstSpell.isEmpty() && !original.equals(firstSpell))
                    result.append(";" + firstSpell);
                String pinyinSpell = Pinyin4jUtil.converterToSpell(name);
